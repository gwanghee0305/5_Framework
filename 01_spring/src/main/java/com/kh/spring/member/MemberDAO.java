...
// DAO : 실제 DB에 접근하는 객체
@Repository		// @Component + DB 접근 계층임을 의미
public class MemberDAO {
		...
		public List<MemberDTO> findAll() {
			...
			String sql = "SELECT ID, NAME, EMAIL, AGE FROM MEMBER ORDER BY ID ASC";
			// JDBC 실행 순서
			// 0) 드라이버 로드
			// 1) Connection 객체			// -> 여기까지 DBUtil 에서 작성함
			// 2) PreparedStatement 객체 생성
			// 3) 쿼리문 실행 후 결과 받기
			//   --> 추출 및 처리 작업
			// 4) 자원 반납 (try ~ with ~ resources 구문 사용 시 생략 가능.) 
			try (Connection conn = DBUtil.getConnection();
				...
				while (rset.next()) {
					...
					// id 컬럼 추출		--> 숫자 / int
					rset.getInt("id");
					// name 컬럼 추출		--> 문자 / String
					rset.getString("name");
					// email 컬럼 추출		--> 문자 / String
		...
		public void delete(int id) {
			// member 테이블에서 회원 번호가 전달된 값(id)와 일치하는 행을 삭제
